...
tarjetaBodys.addEventListener("mouseenter", () => {
    ...
    if (tarjetaConjuntos && imagenConjuntos) { 
...
if (tarjetaLenceria && imagenLenceria) {
...
tarjetaLenceria.addEventListener("mouseenter", () => {
    ...
    if (tarjetaSudadera && imagenSudadera) {
    ...
    const tarjetaEnterizosLargos = tarjetasEnterizosLargos[9];
    ...
    if (tarjetaEnterizosLargos && imagenEnterizosLargos) {
    ...
    if (tarjetaEnterizosCortos && imagenEnterizosCortos) {
    ...
    if (tarjetaEnterizosFaldaShort && imagenEnterizosFaldaShort) {
    ...
    if (tarjetasDeportivosCortos && imagenDeportivosCortos) {
    ...
    if (tarjetaDeportivosLargos && imagenDeportivosLargos) {
    ...
    if (tarjetaTopsDeportivos && imagenTopsDeportivos) {
    ...
    if (tarjetaLeggings && imagenLeggings) {
    ...
    if (tarjetaCamisetas && imagenCamisetas) {
...
const talla = tarjeta.querySelector(".tallas").textContent.replace("Talla:", "").trim();
...
// Guardar automáticamente la página actual
...
if (
    ...
    localStorage.setItem("ultimaCategoria", paginaActual);
}
